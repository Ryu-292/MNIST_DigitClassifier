...
class Model:
  def __init__(self):
    self.layers: list[Callable[[Tensor], Tensor]] = [
            nn.Conv2d(1, 8, kernel_size=3, padding=1), Tensor.silu,
            nn.Conv2d(8, 16, kernel_size=3, padding=1), Tensor.silu,
            lambda x: x.max_pool2d(kernel_size=2, stride=2),

            lambda x: x.flatten(1),
            nn.Linear(16 * 14 * 14, 128), Tensor.silu,
            nn.Linear(128, 10),
    ]
...
if __name__ == "__main__":
  ...
  ANGLE = float(getenv("ANGLE", 15.0))
  SCALE = float(getenv("SCALE", 0.0))
  SHIFT = float(getenv("SHIFT", 0.0))
  ...
  with open("../docs/HYPERPARAMETERS.md", "a") as f:
        f.write(f"| CNN | {B} | {LR} | {getenv('STEPS')} | {getenv('ANGLE', 0)} | {getenv('SCALE', 0)} | {getenv('SHIFT', 0)} | {getenv('SAMPLING', 'NA')} | {best_acc:.2f}% |\n")
